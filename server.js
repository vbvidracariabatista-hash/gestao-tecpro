...
app.post("/orcamentos", (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO orcamentos (cliente, obra, valor_total, dados, criado_em)
    VALUES (?, ?, ?, ?, ?)
  `).run(
    cliente || "",
    obra || "",
...
function getConfig(chave) {
  const c = db.prepare("SELECT valor FROM config WHERE chave=?").get(chave);
...
function getPreco(nome) {
  ...
  const produtos = db.prepare("SELECT nome, preco FROM produtos").all();
...
function getTabela(nome) {
  const item = db.prepare("SELECT valor FROM tabela_precos WHERE nome=?").get(nome);
...
function usarEstoque(material, totalNecessario) {
  ...
  for (let item of sobras) {
    if (item.sobra_mm >= totalNecessario) {
      ...
      if (novaSobra > 0) {
        db.prepare(`
          INSERT INTO estoque (material, sobra_mm)
          VALUES (?, ?)
        `).run(material, novaSobra);
      }
...
function desenharLogoPDF(doc, logoPath, x, y, w, h) {
  ...
  if (fs.existsSync(logoPath)) {
    try {
      ...
      doc.image(logoPath, ix, iy, { width: iw, height: ih });
      doc.restore();
  ...
  textoDentro(doc, "VB", x + 57, y + 18, { bold: true, size: 24, color: "#b30000" });
  textoDentro(doc, "VIDRAÇARIA BATISTA", x + 20, y + 45, { bold: true, size: 10, color: "#003366" });
}
...
function criarPDFCliente(payload, res) {
  ...
  // Cabeçalho
  desenharLogoPDF(doc, logoPath, 25, 25, 155, 78);
